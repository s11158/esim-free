...
export default function CheckoutClient() {
  ...
  useEffect(() => {
    if (!storedOrder || !payment) return;
    const waitingForPayment = payment.status === "pending";
    const waitingForDelivery = payment.status === "paid" && (payment.fulfillmentStatus !== "fulfilled" || payment.emailStatus !== "sent");
    if (!waitingForPayment && !waitingForDelivery) return;
    const poll = window.setInterval(() => void refreshOrder(storedOrder), 10_000);
    return () => window.clearInterval(poll);
  }, [payment, refreshOrder, storedOrder]);
        ...
        <article className="checkout-payment">
          ...
          {payment && liveStatus === "paid" && (
            <div className="crypto-result crypto-result-success" role="status">
              ...
              <p className="esim-email-note">{payment.emailStatus === "sent" ? p.emailed : payment.esim?.qr_code ? p.paidText : p.emailPending}</p>
